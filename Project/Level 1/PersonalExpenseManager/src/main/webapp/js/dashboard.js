  ...
  try {
    ...
    $("category-summary").innerHTML = data.byCategory.length
      ? data.byCategory
          .map(
            (row) =>
              `<div class="summary-row"><span>${esc(row.categoryName)} <small>${row.type === "INCOME" ? "Tiền vào" : "Tiền ra"}</small></span><strong class="${row.type === "INCOME" ? "income-text" : "expense-text"}">${formatVnd(row.total)}</strong></div>`,
          )
          .join("")
